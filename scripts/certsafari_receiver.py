#!/usr/bin/env python3
"""Receive browser-rendered CertSafari questions on loopback and checkpoint them."""
import hashlib
import html
import json
import re
...
ROOT = Path(__file__).resolve().parents[1] / '.corpus' / 'certsafari'
ORIGIN = 'http://127.0.0.1:8768'
EXAMS = {'CCAR-F': 480, 'CCAR-P': 456, 'CCDV-F': 524}
FORM = b'''<!doctype html><title>CertSafari local collector</title>
<h1>CertSafari local collector</h1>
...
def validate_record(record):
    ...
    for key in ('question', 'snapshot', 'source'):
        if not isinstance(record.get(key), str) or not record[key].strip():
            raise ValueError(f'Missing {key}')
    if not record['source'].startswith('https://www.certsafari.com/anthropic/quiz/'):
...
def export_banks():
    for exam, total in EXAMS.items():
        ...
        lines = [f'# {exam} — CertSafari collected questions', '',
        ...
        for index, record in enumerate(records, 1):
            lines.extend([f'## {index}. {record["question"]}', '', f'Source: {record["source"]}', ''])
...
if __name__ == '__main__':
    if sys.argv[1:] == ['--export']:
        ...
        raise SystemExit('Usage: certsafari_receiver.py [--export]')
    else:
