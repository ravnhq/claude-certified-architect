...
// sink: a tab opened at http://127.0.0.1:8768/ by practice_bank_receiver.py.
// Run at most 15 questions per lane per call, then report the saved counts.
function createPracticeBankCollector(lanes, sink) {
  ...
  collector.runLane = async function (lane, count) {
    for (let i = 0; i < count && !lane.end; i++) {
      ...
      try {
        ...
        collector.limit = 'Daily practice limit: resume tomorrow';
        return;
      }
    }
  };
