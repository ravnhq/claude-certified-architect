...
UI = {
    "en": {
        ...
        "focus_report": "Score-report focus",
        "report_note": ("Targeted score-report drill: {n} questions with extra "
                        "weight on weaker reviewed objectives, while retaining "
                        "coverage across all five domains."),
        "report_score": "Targeted practice",
    ...
    "es": {
        ...
        "focus_report": "Enfoque del informe de resultados",
        "report_note": ("Práctica dirigida por el informe: {n} preguntas con "
                        "más peso en los objetivos débiles revisados, manteniendo "
                        "cobertura de los cinco dominios."),
        "report_score": "Práctica dirigida",
    ...
    "pt": {
        ...
        "focus_report": "Foco no relatório de resultados",
        "report_note": ("Treino direcionado pelo relatório: {n} perguntas com "
                        "mais peso nos objetivos fracos revisados, mantendo a "
                        "cobertura dos cinco domínios."),
        "report_score": "Prática direcionada",
...
/* The select carries the exam header's chip metrics, which are two sizes
   smaller than the search box beside it. Both controls state one height here
   so the filter row reads as a single field, not a chip next to an input. */
.browse-head .focus-select { margin-left: 0; height: 36px; padding: 0 10px; }
.browse-search { flex: 1 1 240px; min-width: 0; height: 36px; background: var(--surface); color: var(--fg);
  border: 1px solid var(--border-strong); border-radius: var(--r-md);
...
const BASE_STORE_KEY = "__STOREKEY__";
const TARGETED_MODE = typeof window !== "undefined" &&
  new URLSearchParams(window.location.search).get("targeted") === "1";
const STORE_KEY = TARGETED_MODE ? BASE_STORE_KEY + ":targeted" : BASE_STORE_KEY;
...
function readStored(kind, key) {
  ...
  try { return { available: true, raw: store.getItem(key) }; }
  catch (e) { return { available: false, raw: null }; }
}
function writeStored(kind, key, raw) {
  const store = storage(kind);
...
function loadMisses() {
  try {
    const raw = storage("local")?.getItem(MISS_KEY);
    if (!raw) return {};
...
function saveMisses(items) {
  const raw = JSON.stringify({ v: MISS_VERSION, items: items });
  if (!writeStored("local", MISS_KEY, raw)) {
    persistenceUnavailable = true;
    updateModeHint();
  }
}

let reportRecordCache;
let reportRecordSource = null;
let reportMigratedFromSession = false;
function parseReportRecord(raw) {
  if (!raw) return null;
  try {
    const d = JSON.parse(raw);
    // v1 was the session-only format and implied CCAF-F. v2 carries the
    ...
    const scores = {};
    Object.keys(REPORT_SUBDOMAINS).forEach(id => {
      const value = d.scores[id];
      if (Number.isInteger(value) && value >= 0 && value <= 100) scores[id] = value;
    });
    return Object.keys(scores).length ? { examIdentity: identity, scores: scores } : null;
  } catch (e) { return null; }
}
function serializeReportRecord(record) {
  return JSON.stringify({ v: 2, examIdentity: record.examIdentity, scores: record.scores });
}
function parseHashReportRecord() {
  try {
    if (typeof window === "undefined" || !window.location || !window.location.hash) return null;
    const match = String(window.location.hash).match(/#r=([A-Za-z0-9\-_]+)/);
    if (!match) return null;
    const padded = match[1].replace(/-/g, "+").replace(/_/g, "/");
    const json = decodeURIComponent(escape(atob(padded)));
    return parseReportRecord(json);
  } catch (e) { return null; }
}
function migrateSessionAttempt(forceSession) {
  const session = storage("session");
  if (!session) return false;
  let sessionRaw = null, localRaw = null;
  try {
    sessionRaw = session.getItem(STORE_KEY);
    localRaw = storage("local")?.getItem(STORE_KEY) ?? null;
  } catch (e) {
    persistenceUnavailable = true;
    return false;
  }
  if (!sessionRaw || (!forceSession && localRaw)) return true;
  if (writeStored("local", STORE_KEY, sessionRaw)) return removeStored("session", STORE_KEY);
  persistenceUnavailable = true;
  return false;
}
function reportRecord() {
  if (reportRecordCache !== undefined) return reportRecordCache;
  reportRecordCache = null;
  reportRecordSource = null;
  reportMigratedFromSession = false;
  if (!TARGETED_MODE || !Object.keys(REPORT_SUBDOMAINS || {}).length) return null;

  // A shared custom-exam link carries the report in the URL hash. It wins over
  // stored data so the exact shared set restores, then persists locally.
  const hashed = parseHashReportRecord();
  if (hashed) {
    reportRecordCache = hashed;
    reportRecordSource = "local";
    const canonical = serializeReportRecord(hashed);
    if (!writeStored("local", REPORT_TARGET_KEY, canonical)) persistenceUnavailable = true;
    else removeStored("local", REPORT_CLEARED_KEY);
    removeStored("session", REPORT_TARGET_KEY);
    removeStored("session", STORE_KEY);
    return reportRecordCache;
  }

  const durable = readStored("local", REPORT_TARGET_KEY);
  if (!durable.available) persistenceUnavailable = true;
  const durableRecord = parseReportRecord(durable.raw);
  if (durableRecord) {
    reportRecordCache = durableRecord;
    reportRecordSource = "local";
    const canonical = serializeReportRecord(durableRecord);
    if (durable.raw !== canonical && !writeStored("local", REPORT_TARGET_KEY, canonical)) persistenceUnavailable = true;
    // Once a durable report exists, a session copy is stale and must not win.
    removeStored("session", REPORT_TARGET_KEY);
    removeStored("session", STORE_KEY);
    return reportRecordCache;
  }

  // Explicit deletion leaves a local tombstone so a stale session tab cannot
  // resurrect the report after the durable record has been removed.
  if (readStored("local", REPORT_CLEARED_KEY).raw) {
    removeStored("session", REPORT_TARGET_KEY);
    removeStored("session", STORE_KEY);
    return null;
  }

  const session = readStored("session", REPORT_TARGET_KEY);
  const sessionRecord = parseReportRecord(session.raw);
  if (!sessionRecord) return null;
  reportRecordCache = sessionRecord;
  reportRecordSource = "session";
  reportMigratedFromSession = true;
  const canonical = serializeReportRecord(sessionRecord);
  // Migrate the report and this tab's current targeted attempt together. If
  // an old local attempt exists, session progress is the authoritative one.
  if (writeStored("local", REPORT_TARGET_KEY, canonical)) {
    removeStored("session", REPORT_TARGET_KEY);
    reportRecordSource = migrateSessionAttempt(true) ? "local" : "session";
  } else persistenceUnavailable = true;
  return reportRecordCache;
}
...
function reportScoreSignature() {
  const record = reportRecord();
  return record ? JSON.stringify(Object.keys(record.scores).sort().map(id => [id, record.scores[id]])) : null;
}

function reportWeight(q) {
  const scores = reportScores();
  if (!scores || !q || !q.task_id) return 1;
  const values = Object.keys(REPORT_SUBDOMAINS)
    .filter(id => String(REPORT_SUBDOMAINS[id]) === String(q.task_id) && scores[id] !== undefined)
    .map(id => scores[id]);
  if (!values.length) return 1;
  // Multiple score-report objectives can share one official subdomain. Using
  // the weakest reviewed value keeps that bucket useful without inventing a
  // finer question mapping than the bank actually carries.
  const weakest = Math.min(...values);
  return 1 + (100 - weakest) / 25;
}

function weightedSample(items, count) {
  const pool = items.slice(), picked = [];
  while (pool.length && picked.length < count) {
    const total = pool.reduce((sum, q) => sum + reportWeight(q), 0);
    let cursor = Math.random() * total;
    let index = pool.length - 1;
    for (let i = 0; i < pool.length; i++) {
      cursor -= reportWeight(pool[i]);
...
function drawPerDomain(length, focus) {
  ...
  // The report drill keeps the normal five-domain allocation and only changes
  // which questions are sampled inside each domain.
  if (focus === "report") focus = "all";
  if (focus !== "all") {
    const out = {};
...
// `focus` ("all" | a domain id | "misses" | "report") scopes an attempt: "all" draws
// across every domain, a domain id drills that domain's whole bank, "misses"
// draws the questions this candidate has answered wrong before, and "report"
// keeps the normal domain coverage while weighting weaker reviewed objectives.
// It persists like the attempt length, and the order validation below reads it
// from the payload.
...
// ---- persistence ---------------------------------------------------------
function save() {
  const currentSignature = state.focus === "report" ? reportScoreSignature() : null;
  // A report can be deleted or replaced in another tab while this attempt is
  // open. Refusing this stale write prevents it from recreating old study data.
  if (state.focus === "report" && state.reportSignature !== currentSignature) return false;
...
function load() {
  try {
    let stored = readStored("local", STORE_KEY);
    // A session attempt is only eligible while its report is also being
    // migrated. A durable report makes any session attempt stale.
    if (TARGETED_MODE && reportRecordSource === "session") {
      const sessionStored = readStored("session", STORE_KEY);
      if (sessionStored.raw) stored = sessionStored;
    }
    ...
    if (focus !== "all" && focus !== "misses" && focus !== "report" &&
        !QUESTIONS.some(q => String(q.domain) === String(focus))) return null;
    if (focus === "report") {
      const signature = reportScoreSignature();
      // Pre-persistence targeted attempts had no signature. They are safe to
      // adopt only as part of the paired session migration; every durable
      // attempt written now carries a signature and is invalidated on change.
      if (!signature || (d.reportSignature === undefined && !reportMigratedFromSession) ||
          (d.reportSignature !== undefined && d.reportSignature !== signature)) return null;
      d.reportSignature = signature;
    }
    const ids = new Set(QUESTIONS.map(q => q.id));
    if (!Array.isArray(d.order) || !d.order.every(id => ids.has(id))) return null;
    if (focus === "report" && new Set(d.order).size !== d.order.length) return null;
    if (focus === "misses") {
...
function resetAfterReportStorageChange() {
  if (!TARGETED_MODE) return;
  const signature = reportScoreSignature();
  if (state.focus === "report" && state.reportSignature === signature) return;
  state.focus = signature ? "report" : "all";
...
function onReportStorageChange(event) {
  if (event.key !== REPORT_TARGET_KEY && event.key !== REPORT_CLEARED_KEY) return;
  reportRecordCache = undefined;
  reportRecordSource = null;
  reportMigratedFromSession = false;
  resetAfterReportStorageChange();
}
...
function shuffleOrder() {
  ...
  const order = [];
  const domains = state.focus !== "all" && state.focus !== "report"
  ...
  domains.forEach(d => {
    ...
    if (state.focus === "report") {
      order.push(...weightedSample(ids.map(qById), drawCount(key)).map(q => q.id));
      return;
    }
...
function updateModeHint() {
  ...
  if (!hint) return;
  hint.textContent = (state.mode === "study" ? T.mode_hint_study : T.mode_hint_exam) +
    (persistenceUnavailable ? " Progress could not be saved on this device." : "");
}
...
function updateLengthUI() {
  ...
  const note = state.focus === "misses"
    ...
    : (state.focus === "report"
        ? T.report_note.replace("{n}", examSize())
        : (state.focus !== "all"
...
function setFocus(focus) {
  ...
  if (focus === "report" && !reportScores()) return;
  if (!confirmDiscard()) { updateFocusUI(); return; }
  state.focus = focus;
  state.reportSignature = focus === "report" ? reportScoreSignature() : null;
  restart();
}
...
function clearProgress() {
  const cleared = removeStored("local", MISS_KEY) && removeStored("local", STORE_KEY) &&
    removeStored("session", MISS_KEY) && removeStored("session", STORE_KEY);
  if (!cleared) {
    persistenceUnavailable = true;
    updateModeHint();
  }
...
function fillDomainOptions(sel) {
  ...
  if (reportScores()) {
    ...
    report.value = "report";
    report.textContent = T.focus_report;
    sel.appendChild(report);
  }
...
function updateFocusUI() {
  ...
  if (!sel) return;
  // Weak-spot and score-report drills are not domains, so the selector has no
  // domain value to show while either focused attempt is active.
...
function browseIndex() {
  ...
  BROWSE_INDEX = ordered.map((q, idx) => {
    // Rows are numbered in bank order, not filter order, so "#42" names the
    // same question whatever the reader has typed or scoped to. Typing "#42"
    // in the search box finds it.
    const n = idx + 1;
    ...
    const cn = q.cluster ? size[q.cluster] : 0;
    const ci = q.cluster ? (seen[q.cluster] = (seen[q.cluster] || 0) + 1) : 0;
    return { q: q, n: n, hay: hay, ci: ci, cn: cn };
  });
  return BROWSE_INDEX;
}
...
// A "#42" term names one row exactly, so it does not also surface #420-#429.
function filteredBank(domain, query) {
  ...
  return browseIndex().filter(r =>
    ...
    terms.every(t => /^#\d+$/.test(t) ? String(r.n) === t.slice(1) : r.hay.indexOf(t) >= 0));
}
  ...
  const headHtml = drilling
    ? "<div class='score-grid'>" +
        "<div class='score-card total'><div class='big'>" + correct + "/" + total + "</div>" +
          "<div class='label'>" +
            (weak ? T.misses_score : (state.focus === "report" ? T.report_score : T.drill_score.replace("{d}", state.focus))) +
            " · " + pct + "%</div></div>" +
...
function init() {
  ...
  if (saved) {
    ...
    if (reportScores()) {
      state.focus = "report";
      state.reportSignature = reportScoreSignature();
    }
    state.order = shuffleOrder();
  }
...
def build(lang):
    ...
    common = dict(
        ...
        lang_attr=lang,
        report_map=json.load(open(os.path.join(DATA_DIR, "objectives.json"), encoding="utf-8")).get("objective_subdomains", {}),
    )
    render_page(
        questions=questions,
