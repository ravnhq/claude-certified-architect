...
CCAR-F bank, from the collected practice-bank corpus.

Those two banks are no longer hand-written. Their items come from
.corpus/practice-bank/<CODE>.json — the archive scripts/practice-bank-collection.md
...
What it has to reconcile between the two shapes:
  ...
  * The practice bank ships every option as an "A) text" string and keys an item by
    ...
    letters are reassigned in the order the practice bank presented them. Preserving
  ...
  * A practice-bank stem is one blob; the bank splits scenario from ask because the
  ...
  * Roughly four in five multiple-response stems say "(Select all that apply.)"
    ...
    `stem_note` recording that the edit was ours, not the practice bank's.
  * The bank keys every item to an official blueprint objective. The practice bank
    ...
    list in objectives.json. That is the authority here — it is the practice bank's own
...
Hand-made tags under .corpus/practice-bank/tags/ are honoured as a cross-check when
...
Answers are as the practice bank marked them and have not been independently verified.

Usage: python3 utils/import_practice_bank.py [ccaf|ccap|ccdf|all] [--tags-dir DIR]
...
CORPUS_DIR = os.path.join(ROOT_DIR, ".corpus", "practice-bank")
...
# The practice bank's own label for the item, read out of the captured page snapshot.
...
def split_stem(stem: str) -> tuple[str, str]:
    """Split one practice-bank stem into the bank's `situation` / `question` pair.
...
def state_select_count(question: str, wanted: int, stated_counts) -> tuple[str, str | None]:
    ...
    return (f"{SELECT_ALL.sub('', question)} (Select {wanted}.)",
            f"The practice bank's stem did not state a count; '(Select {wanted}.)' was "
...
def normalized(text: str) -> str:
    ...
    The practice bank abbreviates a long objective title ("Configure Claude tools and
...
def convert(rec: dict, label: dict, stated_counts) -> tuple[dict | None, list[str]]:
    ...
    item = {
        ...
        "id": "",
        "domain": label["domain"],
        # The practice bank's own subdomain id for the item, kept so a reader can trace
...
def import_track(track_key: str, tags_dir: str) -> int:
    ...
    # A stem repeated with different options is one question the practice bank asks
    ...
    for rec in records:
        ...
        if tag and (tag.get("domain"), tag.get("objective")) != \
            ...
            warnings.append(
                ...
                f"{str(tag.get('objective'))!r}, the practice bank's own subdomain "
